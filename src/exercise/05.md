...
- Using {{}} is a combination of JSX expression and object expression. See below if needed.
- If you look in the elements tab, and see something like "== $0", that means the preceeding element can be referenced in the console with $0. So you could do $0.className, for example
The 'class' attribute in normal HTML gets converted to the 'className' DOM property, which is why in JSX we just diretcly use 'className' instead of 'class'. Similarly the DOM prop 'style' is an object where the HTML attribute is a string. But when using JSX we are referencing props, not attribute names.
- WHAT IS THIS MULTIPLE CURSORS WITCHCRAFT?! I MUST LEARN THIS MAGIC
- If you made a 'sizeless' box, then 'undefined' would get put into the className prop that gets passed in, and end up in the className. You can fix it by defaulting the className='' in the params. You could also use .trim to get rid of the space, but no one really cares.
- In his extra credit with the size prop, he left the className prop in the component. That way other people could still pass classNames that have nothing to do with box sizes. I took it out but it make sense that maybe we would have wanted to keep it instead.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=05%3A%20Styling&em=chris.g.peabody%40gmail.com).
