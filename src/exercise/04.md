...
* "Components" are functions which return something that is "renderable," like more react elements, strings, null, numbers, etc
* Using {} with fields directly in the parameter list automatically lets you deconstruct those object fields in the function. It's called "destructuring"
* It seems like the prop types are already a defined thing, so you wouldn't be building out those functions yourself. Fuggin' 'ell, that was the next extra credit
* PropTypes.string doesn't inherently error if the string isn't supplied. I assume that's the same for other types- it probably only errors if you suppply a different type explicitly. But chaining .isRequired will make a warn happen if you don't have anything supplied.
* (PropTypes is less of a thing if you're using typescript)
* using react fragments lets you return more than one element without needing to put them in another element. Stops A node from being created on the DOM

- The 'message' function doesn't actually get called until we tell react to render it. At that point in time, it then asks the function what it contains
- Components and profiler tabs are ones that come with the dev tools extension we got. You can see what components are inside which in the components tab
- Custom components != elements. But they do show up in the components tab. After using the 'message' custom component, you see 'message' with a 'div' inside it in the components tab. But in the actual DOM there are only the divs from inside the message components
- If you name a component with lower case, you'll get a warning in the console. Upper case should be used for react components so things don't get confused with standard HTML elements. This is because JSX with lowercase will convert it to use quotes around it in createElement, whereas uppercase does not
- The way you pass numbers into attributes is to use {} instead of "". Makes sense, just wasn't obvious to me
- Prod version of react does not execture propTypes checks because they are not performant. So you'll only see them while developing
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=chris.g.peabody%40gmail.com).
