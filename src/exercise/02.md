...
- From here forward, if I 'solve' the exercise but it looks pretty different from Kent's solution, I will keep my version just as a reference. I think it will be more useful in seeing what I could have done better
...
- Raw React APIs are a step before using JSX. Christie says JSX is just syntax sugar for making function calls look like you're creating DOM elements (like using brackets and stuff)
- Normally you'd use a package registry like npm to get react and react-dom, but this exercise just pulls the packages directly (so not common practice)

- React.createElement actually just returns an object. So if you use docuement.append to append what is returned, you'll just get the toString of that object. You have to use ReactDOM
- You can use console.log(element) to get info in browser
- React.createElement has a children parameter that you can use instead of passing it in via props. You can actually pass more and more children, so Kent solved this by using that, where I just used a list in the props. He just passed ' ' as one of the children
- There's a warning you get when doing this via props that you don't get when doing it via the prop parameter. Apparently more on this later
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=chris.g.peabody%40gmail.com).
